        ...
        while True:
            ...
            if finder and retry_tpl and (time.time() - last_retry > 15):
                if finder.wait_and_click_template(retry_tpl, timeout=1, times=1, scales=scales):
                    log("🔄 Re-clicked submit button during monitor wait! Aborting — submit still visible.")
                    return "submit_loop"
                last_retry = time.time()
...
def get_project_dir(service=None, fallback=True):
    ...
    if service:
        ...
        # Iterate descending: check most recent dates first
    ...
    # ── 2. Fallback: local standard projects ─────────────────────────────────
...
def main():
    ...
    while True:
        ...
        if do_capture:
            ...
            # ── Submit with 3-attempt retry; exit project if all fail ────────
            submit_tpl = str(script_dir / "realtime" / "submit.png")
            submit_clicked = False
            for attempt in range(1, 4):
                log(f"🖱️ Submit attempt {attempt}/3...")
                if finder.wait_and_click_template(submit_tpl, timeout=10, times=1, scales=search_scales):
                    log(f"✅ Submit clicked on attempt {attempt}.")
                    submit_clicked = True
                    time.sleep(2)
                    break
                else:
                    log(f"⚠️ Submit not found (attempt {attempt}/3).")
                    save_debug_screenshot(f"missing_submit_attempt{attempt}")
                    time.sleep(3)
            ...
            while not capture_done:
                ...
                # ── Automation loop ───────────────────────────────────────────
                try:
                    vb_result = wait_for_visual_begin(
                        monitor, timeout=180,
                    ...
                    if str(e) == "visual_timeout":
                        ...
                        redo_capture = True
                    elif str(e) == "submit_loop":
                        pass  # skip_current_project already set above
                    else:
                        raise e
                finally:
                    log("\n🔌 Stopping recorder...")
